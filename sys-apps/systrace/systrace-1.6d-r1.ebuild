...
# $Header: /usr/local/ssd/gentoo-x86/output/sys-apps/cvs-repo/gentoo-x86/sys-apps/systrace/Attic/systrace-1.6d-r1.ebuild,v 1.1 2006/04/28 13:56:30 lcars Exp $
...
src_unpack() {
	...
	if [ -f /usr/include/linux/systrace.h ] ; then
		SYSTR_HEADER="/usr/include/linux/systrace.h"
	fi
	...
	if ! [ -f ${SYSTR_HEADER} ] ; then
		...
		einfo
		einfo "${SYSTR_HEADER} found. Assuming /dev/systrace."
		einfo
