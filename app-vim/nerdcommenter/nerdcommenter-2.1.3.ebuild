# Copyright 1999-2007 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: /usr/local/ssd/gentoo-x86/output/app-vim/cvs-repo/gentoo-x86/app-vim/nerdcommenter/Attic/nerdcommenter-2.1.3.ebuild,v 1.2 2007/09/07 18:35:12 hawking Exp $

inherit vim-plugin

DESCRIPTION="vim plugin: easy commenting of code for many filetypes."
HOMEPAGE="http://www.vim.org/scripts/script.php?script_id=1218"
LICENSE="public-domain"
KEYWORDS="~x86"
IUSE=""

VIM_PLUGIN_HELPFILES="NERD_commenter"
