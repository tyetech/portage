# Copyright 1999-2006 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: /usr/local/ssd/gentoo-x86/output/app-vim/cvs-repo/gentoo-x86/app-vim/detectindent/detectindent-1.0.ebuild,v 1.5 2006/01/24 17:10:18 chutzpah Exp $

inherit vim-plugin

DESCRIPTION="vim plugin: automatically detect file indent settings"
HOMEPAGE="http://www.vim.org/scripts/script.php?script_id=1171"
LICENSE="vim"
KEYWORDS="amd64 ~hppa mips ~ppc sparc x86"
IUSE=""

VIM_PLUGIN_HELPFILES="detectindent.txt"

